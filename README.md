...
```
curl -v -F 'metadata={"mode":1};type=application/json' -F file=@<video_file> --insecure https://<host>:9090/upload
```
where \<video_file\> is the path to the video you want to analyze, and \<host\> is the host the backend running on. Note that we add `--insecure` to make `curl` trusts our self-signed certificate.
